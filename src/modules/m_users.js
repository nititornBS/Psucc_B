...
module.exports = {
  ...
  mActivationUser: (email, access) => {
    return new Promise((resolve, reject) => {
      pool.query('UPDATE users SET , status = ?, access = ? WHERE email = ?', [ 1, access, email], (err, result) => {
        if (!err) {
          resolve(result);
        } else {
          reject(new Error(err));
  ...
  mProfileMe: (token) => {
    return new Promise((resolve, reject) => {
      pool.query(`SELECT id, name, email, company_id, token, expired, image, status, access, created, updated FROM users WHERE token='${token}'`, (err, result) => {
        if (!err) {
          resolve(result);
        } else {
          reject(new Error(err));
  ...
  mDetailUser: (id) => {
    return new Promise((resolve, reject) => {
      pool.query(`SELECT id, name, email, company_id, token, expired, image, status, access, created, updated FROM users WHERE id='${id}'`, (err, result) => {
        if (!err) {
          resolve(result)
        } else {
          reject(new Error(err))
