  ...
  register: async (req, res) => {
    try {
      ...
      if (emailCheck.length >= 1) {
        ...
        // Create token
        const token = jwt.sign({ name: body.name, create: new Date() }, envJWTKEY);

        // Register user
        await mRegister(userData);

        // Create activation
        await mCreateActivation(token, body.email);

        // Omitting email sending since activation status is set directly
        success(res, 'Registration successful! Activate your account through other means.', {}, null);
      }
    } catch (error) {
